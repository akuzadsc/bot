...
ROLE_ID = 1245877322721263616  
CHANNEL_ID = 1246000024874123345  
LOG_CHANNEL_ID = 1246547597259440179  
BL_ROLE_ID = 1246544747041128562 
...
async def on_member_update(before, after):
    ...
    if role is not None:
        if "❌" in before.display_name and "❌" not in after.display_name:
            if role in before.roles:
                await after.remove_roles(role)
                message = f"{after.mention} a perdu le rôle {role.name}"
                print(message)
        elif "❌" not in before.display_name and "❌" not in after.display_name:
...
client.run('MTI0NjE4NDAwMzc1MzQ4MDI3NQ.G27Z-o.xRt2voWP5leZB_XG7A14sXh1jYF9oAvTAk58kQ')
